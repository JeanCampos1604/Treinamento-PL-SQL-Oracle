...
SELECT O.BUYER_ID "buyer_id", U.JOIN_DATE "join_date" , COUNT(O.ORDER_DATE) "orders_in_2019"
  FROM USERS2 U, ORDERS O
  WHERE U.USER_ID = O.BUYER_ID 
  AND O.ORDER_DATE BETWEEN '01/01/2019' AND '31/12/2019'
  GROUP BY buyer_id,join_date
  UNION
  SELECT O.BUYER_ID "buyer_id", U.JOIN_DATE "join_date" , COUNT(O.ORDER_DATE) - COUNT(O.ORDER_DATE) "orders_in_2019"
  FROM USERS2 U, ORDERS O
  WHERE U.USER_ID = O.BUYER_ID
  AND O.ORDER_DATE NOT BETWEEN '01/01/2019' AND '31/12/2019'
  GROUP BY buyer_id,join_date
  ORDER BY 1;